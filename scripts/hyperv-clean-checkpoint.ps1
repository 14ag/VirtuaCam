...
$artifactDir = if ([string]::IsNullOrWhiteSpace($ArtifactRoot)) { Get-HvArtifactDirectory -ArtifactRoot "test-reports\hyperv-clean" } else { Resolve-HvPath -Path $ArtifactRoot }
