...
$artifactDir = if ([string]::IsNullOrWhiteSpace($ArtifactRoot)) {
    Join-Path $repoRoot ("test-reports\vhlk-all-export-{0}" -f (Get-Date -Format "yyyyMMdd-HHmmss"))
} else {
...
try {
    ...
    if (-not $SkipFreshStart) {
        Write-HvLog -Message ("Fresh-starting vHLK controller '{0}' before all-name export." -f $VhlkVmName) -LogPath $logPath -Level STEP
    ...
    $status = Invoke-Command -Session $session -ScriptBlock {
        ...
        try {
            ...
            $allNames = @($testRows | ForEach-Object { [string]$_.Name } | Sort-Object -Unique)
    ...
    @($status.AllTestNames) | Set-Content -LiteralPath (Join-Path $artifactDir "all-test-names.txt") -Encoding UTF8
    Write-Host ("Exported {0} all names from {1} tests; passed={2}, failed={3}, notrun={4}, running={5}." -f @($status.AllTestNames).Count, $status.Total, $status.Passed, $status.Failed, $status.NotRun, $status.Running)
