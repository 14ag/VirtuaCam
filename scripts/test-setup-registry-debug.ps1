...
Assert-Contains "wizard-project\src\VirtuaCamSetup.cpp" "MoveWindow(g_status, 0, 0, 0, 0, FALSE)" "Setup status text must not appear under preview"
Assert-Contains "wizard-project\src\VirtuaCamSetup.cpp" "MoveWindow(g_checks, 0, 0, 0, 0, FALSE)" "Setup check list rectangle must not appear under preview"
Assert-NotContains -Paths @((Join-Path $repoRoot "wizard-project\src\VirtuaCamSetup.cpp")) -Pattern "WS_CHILD \| WS_VISIBLE \| SS_LEFT" -Message "Setup must not show status text under preview"
Assert-NotContains -Paths @((Join-Path $repoRoot "wizard-project\src\VirtuaCamSetup.cpp")) -Pattern "WS_CHILD \| WS_VISIBLE \| WS_BORDER \| WS_TABSTOP \| WS_VSCROLL" -Message "Setup must not show checklist rectangle under preview"
Assert-Contains "wizard-project\src\VirtuaCamSetup.cpp" "Successfully installed drivers" "Setup install success popup text missing"
