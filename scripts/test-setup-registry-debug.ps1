...
Assert-NotContains -Paths @((Join-Path $repoRoot "scripts\build-all.ps1")) -Pattern '[switch]$Clean' -Message "Build clean mode must be default, not a user flag"
Assert-NotContains -Paths @((Join-Path $repoRoot "scripts\build-all.ps1")) -Pattern '$Clean' -Message "Build clean mode must be unconditional, not flag-driven"
Assert-Contains "scripts\build-all.ps1" '$targets = "Clean;Build"' "Build must run clean targets by default"
...
Assert-Contains "wizard-project\src\VirtuaCamSetup.cpp" "WriteHeadlessSummary(result)" "Setup headless mode must write console summary"
Assert-Contains "wizard-project\src\VirtuaCamSetup.cpp" "WriteStdHandleLine(STD_OUTPUT_HANDLE, line)" "Setup headless mode must mirror output to stdout"
Assert-Contains "wizard-project\src\VirtuaCamSetup.cpp" "WriteStdHandleLine(STD_ERROR_HANDLE, line)" "Setup headless mode must mirror output to stderr"
Assert-NotContains -Paths @((Join-Path $repoRoot "wizard-project\src\VirtuaCamSetup.cpp")) -Pattern "--quiet" -Message "Setup must not expose a quiet flag"
Assert-NotContains -Paths @((Join-Path $repoRoot "wizard-project\src\VirtuaCamSetup.cpp")) -Pattern "/quiet" -Message "Setup must not expose a quiet slash flag"
