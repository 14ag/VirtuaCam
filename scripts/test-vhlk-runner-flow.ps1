...
Assert-Contains -Path "scripts\install-driver-for-vhlk.ps1" -Pattern "Set-VhlkCameraDirectMode" -Message "vHLK DUT install must configure camera direct mode before HLK reliability tests."
...
Assert-Contains -Path "scripts\install-driver-for-vhlk.ps1" -Pattern "EnableFrameServerMode" -Message "vHLK DUT install must disable Media Foundation frame-server brokering."
Assert-Contains -Path "scripts\install-driver-for-vhlk.ps1" -Pattern "OpenBaseKey" -Message "vHLK DUT install must use registry API for idempotent direct-mode keys."
Assert-Contains -Path "scripts\install-driver-for-vhlk.ps1" -Pattern "CreateSubKey" -Message "vHLK DUT install must create direct-mode registry keys without provider delete/recreate behavior."
...
Assert-Contains -Path "scripts\install-driver-for-vhlk.ps1" -Pattern 'camera-frame-server-mode\.json' -Message "vHLK DUT install must persist camera direct-mode state."
