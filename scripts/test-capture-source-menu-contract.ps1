...
if ($windowPos -lt 0 -or $displayPos -lt 0 -or $cameraPos -lt 0 -or $imagePos -lt 0) {
    throw "Source menu sections missing."
}
if (-not ($windowPos -lt $displayPos -and $displayPos -lt $cameraPos -and $cameraPos -lt $imagePos)) {
    throw "Source menu order is not window, display, camera, file."
}
