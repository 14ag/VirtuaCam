...
try {
    ...
    $install = Invoke-HvGuestCommand -Session $session -LogPath $logPath -ScriptBlock {
        ...
        $process = Start-Process -FilePath $SetupExe -ArgumentList @("--install", "--skip-watcher-service", "--json", $JsonPath) -Wait -PassThru -WindowStyle Hidden -RedirectStandardOutput $stdout -RedirectStandardError $stderr
        $lines = @()
