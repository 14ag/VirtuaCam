...
DWORD g_DisableToneGenerator = 1;  // VirtuaCam mic is fed by the IOCTL bridge; underruns emit silence.
...

{
    ...
    {
        DPF(D_VERBOSE, ("IoOpenDriverRegistryKey failed, using defaults, 0x%x", ntStatus));
        return STATUS_SUCCESS;
    }
...
--*/
    ...
    //
    ntStatus = GetRegistrySettings(DriverObject);
    IF_FAILED_ACTION_JUMP(
        ntStatus,
    ...
    DriverObject->MajorFunction[IRP_MJ_CREATE] = VirtuaCamMicBridgeDispatchCreateClose;
    DriverObject->MajorFunction[IRP_MJ_CLOSE] = VirtuaCamMicBridgeDispatchCreateClose;
    DriverObject->MajorFunction[IRP_MJ_CLEANUP] = VirtuaCamMicBridgeDispatchCreateClose;
    DriverObject->MajorFunction[IRP_MJ_DEVICE_CONTROL] = VirtuaCamMicBridgeDispatchDeviceControl;
#endif

    ntStatus = VirtuaCamMicBridgeInitialize(DriverObject);
    IF_FAILED_ACTION_JUMP(
        ntStatus,
        DPF(D_ERROR, ("VirtuaCamMicBridgeInitialize failed, 0x%x", ntStatus)),
        Done);
    ...
    )
{
    ...
    UNREFERENCED_PARAMETER(ppAeMiniports);
#endif
